...
4. **Rutas nuevas en `apps/web/app/`:**
   ...
   - `/auth/confirm` (**Route Handler**, no página) — a donde apuntan los enlaces de los emails de Supabase (confirmación de registro y recuperación de contraseña). Verifica el `token_hash` con `supabase.auth.verifyOtp(...)` y redirige a `/cuenta` o `/restablecer-contrasena` según el tipo. Sin esto, el enlace del email no tiene dónde aterrizar — descubierto al planificar los Server Actions, no estaba en la primera versión de este plan. Requiere además un paso manual en el panel de Supabase: cambiar la plantilla de esos emails (Authentication → Email Templates) para que enlacen a esta ruta en vez del enlace por defecto de Supabase.
...
6. **`proxy.ts`** en la raíz de `apps/web` — comprueba la sesión antes de servir rutas privadas (empieza solo con `/cuenta`; cuando lleguen 012+, se amplía la lista de rutas protegidas aquí mismo, sin tocar cada página). Se llama `proxy.ts` y no `middleware.ts`: en Next.js 16 (la versión de este proyecto) se renombró el archivo y la función exportada (`middleware` → `proxy`) — el nombre viejo sigue funcionando pero está marcado como obsoleto.
